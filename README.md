...
- 🧬 **无文件落地启动** — 内核配置通过 stdin 管道注入（Xray/Sing-box/Mihomo），全程不触碰物理磁盘，零 I/O 开销，保护敏感信息，目录整洁无残留
- 🧠 **协议→内核智能选择** — 后端能力矩阵一次性下发，节点编辑时协议切换自动推荐最佳已安装内核
- 🐾 **Mihomo 完整支持** — Clash Meta 内核的完整 YAML 配置生成，支持 8 种协议 + TLS/Reality
- ✏️ **失焦自动保存** — 设置页面移除全局保存按钮，输入框失焦/回车/Toggle 即时保存，后端脏标记零 I/O 拦截
- 🔐 **安全认证** — JWT 用户认证 + TOTP 两步验证（支持自定义密钥 + Base32 校验 + 失焦自动回滚）+ bcrypt 密码哈希 + 自签证书 HTTPS + JWT 过期时间可配置（内存缓存即时生效）
